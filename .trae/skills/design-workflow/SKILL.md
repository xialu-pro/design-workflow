...
description: "End-to-end workflow orchestrator: PRD generation (requirement-doc-generator) → design draft generation (design-draft-generator, route A/B) → adversarial review → PRD backfill. Invoke when user asks to run the full 需求到设计稿 flow or mentions 设计工作流/端到端."
---

# 设计工作流（需求 → 设计稿 端到端编排）

串联 requirement-doc-generator 与 design-draft-generator 两个 skill，把"想法 → PRD → 设计稿 → 闭环回填"跑成一条流水线。本 skill **只负责编排**（阶段流转、确认门、交接物管理），不重复两个执行 skill 的内部规范——执行时严格按各 skill 自身的工作流走。
...
4. 输出最终交付摘要：PRD 路径 + 版本、设计稿路径清单、评审报告结论、回填位置——即交接物清单的最终态
...
三个 skill 可独立调用，也可由本 skill 编排为端到端流水线；独立使用时不强制经过本工作流。
