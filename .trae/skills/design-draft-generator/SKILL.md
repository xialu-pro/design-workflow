...
将简报模板 [templates/new-community-brief.md](templates/new-community-brief.md) 交给设计师填写；设计师不便填写时，按简报结构（任务 / 目标用户 / 内容域 / 内容素材 / 关键功能 / 风格与方案数量）通过 AskUserQuestion 代填。内容素材越真实评审越有效（Developers over sales：用数字，不用形容词），无素材条目留空以占位内容补齐。
